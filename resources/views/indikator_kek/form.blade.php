...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
  ...
  <div class="flex items-center justify-between">
    ...
    <a href="{{ route('ibu-hamil-kek.index', ['tahun' => $return_year ?? now()->year]) }}"
  ...
  <form action="{{ $action }}" method="POST" class="bg-white rounded-xl shadow-sm border p-4 space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        ...
        <input type="number" name="tahun" value="{{ old('tahun', $item->tahun ?? now()->year) }}"
               class="w-full rounded-md border-gray-300" min="2000" max="2100" required>
      </div>
      ...
      <div class="md:col-span-2">
        ...
        <input type="number" name="jumlah" value="{{ old('jumlah', $item->jumlah ?? 0) }}"
               class="w-full rounded-md border-gray-300" min="0" required>
      </div>
    </div>
