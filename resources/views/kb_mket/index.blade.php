...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">Kelola data peserta KB MKET per bulan</p>
        </div>

        <div class="flex items-center gap-2">
            {{-- Filter Tahun --}}
            <form method="GET" action="{{ route('kb-mket.index') }}" class="flex items-center gap-2">
            ...
            <a href="{{ route('kb-mket.create', ['tahun' => $selected_year]) }}"
                    ...
                    <tr class="hover:bg-gray-50/60">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="{{ route('kb-mket.edit', ['kb_mket' => $item->id, 'tahun' => $selected_year]) }}"
                                   class="px-3 py-1.5 rounded-lg bg-sky-50 text-sky-700 hover:bg-sky-100">
                                    Edit
                                </a>

                                <form action="{{ route('kb-mket.destroy', ['kb_mket' => $item->id, 'tahun' => $selected_year]) }}"
                                      method="POST"
