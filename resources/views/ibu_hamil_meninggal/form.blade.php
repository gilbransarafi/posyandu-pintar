...
@section('title', $title ?? 'Form Ibu Hamil Meninggal')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">{{ $title ?? 'Form Data' }}</h1>
            <p class="text-sm text-gray-500">Isi data tahun, bulan, dan jumlah</p>
        </div>

        <a href="{{ route('ibu-hamil-meninggal.index', ['tahun' => $return_year ?? now()->year]) }}"
    ...
    <form method="POST" action="{{ $formAction }}" class="bg-white rounded-xl shadow-sm border p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div class="md:col-span-2">
                <label class="text-sm text-gray-600">Jumlah Ibu Hamil Meninggal</label>
